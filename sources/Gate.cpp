...
Gate::Gate(string type, vector<string> i, string o, int d)
    : operationType(type), inputs(i), output(o), delay(d) {}
...
int Gate::evaluate() {
    ...
    if (operationType == "or") {
        ...
        for (int i = 0; i < inputs.size(); ++i) {
            result =
                result ||
                mp[inputs[i]]; // update the result one by one from the inputs
        }
        return result;
        ...
        // xnor: result is true if an even number of inputs are true (inverse of
        // XOR)
        bool result = false;
        for (int i = 0; i < inputs.size(); ++i) {
